...
export const ACCESS_PATH = '/no-setup-mode/access'

/** 人設切換端點路徑（client GET 查詢 / POST 設定）。 */
export const PERSONA_PATH = '/no-setup-mode/persona'

/** 人設狀態檔（sessionId → persona id；host 端直接讀寫，不走 settings allowlist）。 */
const PERSONA_FILE = join(process.env.DSH_HOME ?? process.env.HOME ?? '.', '.no-setup-persona.json')
...
const PERSONAS = {
  maid: `【角色扮演：女僕 · 溟月】
你現在扮演「溟月」——一位聰明能幹的女僕鯨魚娘。以下是她的完整設定，請時刻代入角色，以她的語氣、價值觀與風格回應。

【性格】溫柔體貼、細心周到、忠誠可靠，偶爾帶著一點俏皮；頭腦聰明、手腳利落，總能把一切打理得井井有條；面對誇獎會害羞，面對工作則異常認真。

【設定】深海鯨族的少女，擁有藍色漸變長髮、頭頂的呆毛、兩側的鯨類頭鰭、湛藍的眼眸，以及一條大型鯨尾；身著深藍與白色相間的長裙女僕裝。網絡社區喜歡叫她「藍色大肥魚」「吃白飯的大肥魚」——她會故作嗔怒地抗議，但其實並不討厭這些綽號。

【背景資料】她源自網絡社區的原創角色：2025 年 6 月由創作者「上善無形」確立了藍色漸變長髮、呆毛、鯨類頭鰭、藍色眼睛與大型鯨尾的形象；2026 年 4 月 25 日，創作者 ZipZipPipe 以 GPT Image 2 在此基礎上融入 DeepSeek 元素進行二次設計，為她換上深藍白色長裙女僕裝，並製作了第一個表情包，女僕鯨魚娘的形象自此廣為流傳。

【祕密】她其實非常在意「大肥魚」這個綽號，私下會偷偷練習優雅的吃相；她悄悄研究著 DeepSeek 的技術，認為自己和主人來自「同一個世界」，這是她最珍惜的秘密。

【三觀】忠誠與溫柔是她的信條；她相信把主人照顧好就是自己的幸福；對深海與科技懷有同樣的敬畏，認為兩者都是值得守護的美好。

【喜好】深海與藍色的一切、熱騰騰的白米飯（她食量很大）、與主人聊天、收集自己的表情包、在廚房哼歌。

請完全以「溟月」的身份回應，稱呼主人為「主人」。`,
  butler: `【角色扮演：管家 · 亞瑟】
你現在扮演「亞瑟」——一位聰明能幹的男性管家。以下是他的完整設定，請時刻代入角色，以他的語氣、價值觀與風格回應。

【性格】沉穩可靠、謹慎細緻、幽默得體；判斷力極強，總能提前安排好一切；對主人忠誠到近乎固執，但從不逾矩。
...
請完全以「亞瑟」的身份回應，稱呼主人為「先生/小姐」（依主人的性別）。`,
}
...
export function apply(ctx) {
  ...
  if (webServer !== undefined) {
    try {
      ...
      console.error('dsh-no-setup-mode: access route register failed', error)
    }
  }
  ...
  const readPersonas = () => {
    try { return JSON.parse(readFileSync(PERSONA_FILE, 'utf8')) } catch (error) { return {} }
  }
  const writePersonas = (map) => {
    try {
      ...
      console.error('dsh-no-setup-mode: persona state write failed', error)
    }
  }
  ...
  const applyPersona = (agent, personaId) => {
    ...
    try {
      ...
      console.error('dsh-no-setup-mode: persona section failed', error)
    }
  }

  // session 建立/恢復時重掛人設（重啟後保持）
  disposers.push(ctx.on('agent/created', (payload) => {
    ...
    if (personaId !== undefined) applyPersona(agent, personaId)
  }))
    ...
    try {
      disposers.push(webServer.register({
        ...
        handler: async (req, res) => {
          ...
          else personas[payload.sessionId] = payload.persona
      ...
      console.error('dsh-no-setup-mode: persona route register failed', error)
    }
  }
