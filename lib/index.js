...
/**
 * 極簡專屬 Agent 預設（名稱中英雙語）：
 * 聊天 = 極簡 + 聯網 + Flash（思考 off）；工作 = 標準 + Flash（思考 high）；
 * 專家 = PTC + Pro（思考 max）。
 */
export const MINIMAL_PRESETS = [
  { id: 'chat', from: 'minimal', name: '極簡：聊天模式（Minimal Chat Mode）', extraWeb: true, persistent: true },
  { id: 'work', from: 'standard', name: '極簡：工作模式（Minimal Work Mode）' },
  { id: 'expert', from: 'code', name: '極簡：專家模式（Minimal Expert Mode）' },
]
...
export const MODEL_PATH = '/minimal-mode/model'
...
export function apply(ctx) {
  ...
  // 確保三個極簡 preset 存在（不存在才複製；名稱過時（舊版）時重建；聊天模式額外補聯網工具）
  ...
  // ---- 模型切換端點：POST /minimal-mode/model ----
  ...
  if (webServer !== undefined) {
    try {
      ...
      console.error('dsh-minimal-mode: model route register failed', error)
    }
  }
