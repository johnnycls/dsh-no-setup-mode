 ...
 */
export const inject = ['settings', 'credentials', 'agentPresets', 'webServer', 'sessions', 'sandboxPolicy']
...
export function apply(ctx) {
      ...
      disposers.push(webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            // 已是目標模式時不重複 append（避免 sandbox/mode 事件膨脹，以及持久終端
            // 開啟時切換拋錯導致的 500）
            const current = sandboxPolicy !== undefined ? sandboxPolicy.overrideOf(session) : undefined
            if (current === mode) {
              return send(200, { ok: true, sessionId: payload.sessionId, mode, unchanged: true })
            }
  ...
  // agents（agent-loop）啟動較慢（恢復長 session）——未就緒時定時重試，不阻塞路由註冊。
  const hangBootPersonas = () => {
    const agentsNow = ctx.get('agents')
    if (agentsNow === undefined) return false
    for (const agent of agentsNow.list()) applyPersonaForAgent(agent)
    return true
  }
  if (!hangBootPersonas()) {
    let hangAttempts = 0
    const hangTimer = ctx.setInterval(() => {
      hangAttempts += 1
      if (hangBootPersonas() || hangAttempts >= 30) hangTimer()
    }, 2000)
    disposers.push(hangTimer)
  }
    ...
    try {
      disposers.push(webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const agent = agentsService !== undefined ? agentsService.get(payload.sessionId) : undefined
