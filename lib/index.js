...
/**
 * 免設置專屬 Agent 預設（名稱中英雙語）：
 * 聊天 = 免設置 + 聯網 + Flash（思考 off）；工作 = 標準 + Flash（思考 high）；
 * 專家 = PTC + Pro（思考 max）。
 */
export const MINIMAL_PRESETS = [
  { id: 'chat', from: 'no-setup', name: '免設置：聊天模式（No-Setup Chat Mode）', extraWeb: true, persistent: true },
  { id: 'work', from: 'standard', name: '免設置：工作模式（No-Setup Work Mode）' },
  { id: 'expert', from: 'code', name: '免設置：專家模式（No-Setup Expert Mode）' },
]
