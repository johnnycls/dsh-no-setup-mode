  ...
  factory: (require) => {
    ...
    return {
      ...
      apply(ctx) {
        ...
        // 重啟後瀏覽器可能早於 DSH 啟動完成而載入（插件在組合末尾 apply）——
        // 此期間路由未註冊會 404；以 3 秒間隔重試，覆蓋 60 秒啟動窗口後自動成功。
        const setSessionAccess = (sessionId, mode, attempt = 0) => {
          ...
          fetch(`/${MODE.code}/access`, {
            ...
            if (!res.ok && attempt < 20) {
              setTimeout(() => { setSessionAccess(sessionId, mode, attempt + 1) }, 3000)
            }
          }).catch(() => {
            if (attempt < 20) setTimeout(() => { setSessionAccess(sessionId, mode, attempt + 1) }, 3000)
          })
        }
