  ...
  factory: (require) => {
    ...
    return {
      ...
      apply(ctx) {
        ...
        // Full Access：進入免設置模式時把當前 session 切到 danger-full-access（免審核），離開時恢復默認
        const setSessionAccess = (mode) => {
          if (api === undefined) return
          rpc(api.sessions.list({})).then((res) => {
            ...
            if (current === undefined || current.sessionId === undefined || current.sessionId === null) return
            fetch('/no-setup-mode/access', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ sessionId: current.sessionId, mode }),
            }).catch(() => {})
          }).catch(() => {})
        }
        ...
        function MinimalHeader(props) {
          ...
          const presets = [
            { id: 'chat', name: '聊天', full: '免設置：聊天（Chat）' },
            { id: 'work', name: '工作', full: '免設置：工作（Work）' },
            { id: 'expert', name: '專家', full: '免設置：專家（Expert）' },
          ]
          ...
          React.useEffect(() => {
            ...
            let alive = true
            fetch(`/no-setup-mode/persona?sessionId=${encodeURIComponent(sid)}`, { cache: 'no-store' })
              .then((res) => res.json())
              .then((data) => {
          ...
          const personaOptions = [
            { id: 'none', name: '無人設', full: '無人設（不注入角色扮演提示詞）' },
            { id: 'maid', name: '女僕', full: '女僕 · 溟月（鯨魚娘）' },
            { id: 'butler', name: '管家', full: '管家 · 亞瑟（男性）' },
          ]
          ...
          return React.createElement('div', {
            ...
            React.createElement('div', {
              ...
              React.createElement('span', { style: { fontSize: 12, lineHeight: '26px', color: 'var(--dsw-alias-label-secondary)' } }, '人設'),
