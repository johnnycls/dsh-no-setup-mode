  ...
  factory: (require) => {
    ...
    return {
      ...
      apply(ctx) {
        ...
        // 套用極簡偏好：busyEnter=steer（插話）、default=chat + 聊天模式（思考 off）；並記錄原值
        ...
        function MinimalHeader(props) {
          ...
          const presets = [
            { id: 'chat', name: '極簡：聊天（Chat）', full: '極簡：聊天模式（Minimal Chat Mode）' },
            { id: 'work', name: '極簡：工作（Work）', full: '極簡：工作模式（Minimal Work Mode）' },
            { id: 'expert', name: '極簡：專家（Expert）', full: '極簡：專家模式（Minimal Expert Mode）' },
          ]
