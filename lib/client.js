  ...
  factory: (require) => {
    ...
    return {
      ...
      apply(ctx) {
        ...
        // 系統設置 tabs（settings.section 的 shipped 註冊）——免設置設置頁要隱藏它們
        const SHIPPED_SECTION_IDS = ['general', 'models', 'plugins', 'agent-presets']
        ...
        const STRINGS = {
          zh: {
            ...
            settingsTab: '免設置',
            pluginTabFailed: '此設置頁無法在此顯示（插件相容性）',
          },
          en: {
            ...
            settingsTab: 'No-Setup',
            pluginTabFailed: 'This settings page cannot be shown here (plugin compatibility)',
          },
        }
        ...
        // 重啟後瀏覽器可能早於 DSH 啟動完成而載入（插件在組合末尾 apply，長 session
        // 恢復可達 1-2 分鐘）——此期間路由未註冊會 404。線性退避重試（3s/6s/9s…，
        // 最多 10 次、總窗口約 165 秒）：請求少（降低 console 噪音）且覆蓋完整窗口。
        const setSessionAccess = (sessionId, mode, attempt = 0) => {
          ...
          fetch(`/${MODE.code}/access`, {
            ...
            if (!res.ok && attempt < 10) {
              setTimeout(() => { setSessionAccess(sessionId, mode, attempt + 1) }, (attempt + 1) * 3000)
            }
          }).catch(() => {
            if (attempt < 10) setTimeout(() => { setSessionAccess(sessionId, mode, attempt + 1) }, (attempt + 1) * 3000)
          })
        }
        ...
        // 免設置設置內容（語言 / API Key / 離開）——作為「免設置」tab 內容，
        // 同時註冊為 settings.section（普通模式的 shipped 設置面板也會顯示此 tab）
        ...
        // 插件設置 tab 內容渲染失敗時降級顯示（避免整個設置頁崩潰）
        class SettingsTabBoundary extends React.Component {
          ...
          componentDidCatch(error) {
            console.error(`${MODE.code}: plugin settings tab failed`, error)
          }
          render() {
            if (this.state.failed) {
              const fallback = (() => { try { return locale.getSnapshot().active === 'en' ? STRINGS.en.pluginTabFailed : STRINGS.zh.pluginTabFailed } catch (e) { return STRINGS.zh.pluginTabFailed } })()
              return React.createElement('p', { style: { margin: 0, fontSize: 13, lineHeight: '20px', color: 'var(--dsw-alias-state-error-primary)' } }, fallback)
            }
        ...
        function MinimalSettingsShell(props) {
          ...
          const [active, setActive] = React.useState('no-setup-mode')
          const [pluginTabs, setPluginTabs] = React.useState([])
          const { t } = useLocale()
          const wide = props.wide === true

          // 插件設置 tabs：settings.section 的註冊項，排除系統 tabs（general/models/plugins/agent-presets）
          const refreshTabs = React.useCallback(() => {
            try {
              const entries = slots.entries('settings.section')
              const list = entries
                .filter((e) => SHIPPED_SECTION_IDS.indexOf(e.options.id) === -1 && e.options.id !== MODE.code)
                .map((e) => ({
                  id: e.options.id,
                  label: typeof e.options.label === 'function' ? e.options.label() : (e.options.label !== undefined ? e.options.label : e.options.id),
                  component: e.component,
                }))
              setPluginTabs(list)
            } catch (err) { /* slots 不可用時忽略 */ }
          }, [])

          React.useEffect(() => {
            refreshTabs()
            let unsub = () => {}
            try { unsub = slots.subscribe('settings.section', refreshTabs) } catch (err) {}
            return () => { unsub() }
          }, [refreshTabs])
          ...
          const tabs = [
            { id: 'no-setup-mode', label: t('settingsTab') },
            ...pluginTabs,
          ]
          ...
          const panel = React.createElement('div', {
            ...
            React.createElement('div', {
              style: {
                ...
                width: 720, maxWidth: 'calc(100vw - 48px)', height: 'min(680px, calc(100vh - 48px))',
              ...
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 54, padding: '0 24px', boxSizing: 'border-box', flex: 'none' } },
              ...
              React.createElement('div', { style: { display: 'flex', flex: 1, minHeight: 0 } },
                React.createElement('div', {
                  style: { boxSizing: 'border-box', width: 172, flex: 'none', borderRight: '1px solid var(--dsw-alias-border-l1)', padding: '8px 8px 16px', display: 'flex', flexDirection: 'column', gap: 4, overflowY: 'auto' },
                },
                  tabs.map((tab) => {
                    ...
                    return React.createElement('button', {
                      ...
                      style: {
                        boxSizing: 'border-box', display: 'block', width: '100%', textAlign: 'left',
                        height: 36, padding: '0 12px', border: 'none', borderRadius: 10, cursor: 'pointer',
                        font: 'inherit', fontSize: 14, lineHeight: '36px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                ...
                React.createElement('div', { style: { flex: 1, minWidth: 0, padding: '4px 24px 24px', overflowY: 'auto' } },
                  active === 'no-setup-mode'
                    ? React.createElement(MinimalSettingsContent)
                    : (() => {
                      const tab = pluginTabs.find((p) => p.id === active)
                      if (tab === undefined || typeof tab.component !== 'function') return null
                      return React.createElement(SettingsTabBoundary, null,
                        React.createElement(tab.component, { close: () => setOpen(false) }),
                      )
                    })(),
                ),
              ),
        ...
        // 免設置設置 tab（settings.section）：普通模式的 shipped 設置面板也會顯示此 tab；
        // 免設置模式下 MinimalSettingsShell 的 tabs 容器會列出它與其他插件 tabs。
        slots.inject('settings.section', () => slots.register({
          name: 'settings.section',
          id: MODE.code,
          order: 50,
          label: () => {
            try { return locale.getSnapshot().active === 'en' ? 'No-Setup' : '免設置' } catch (e) { return '免設置' }
          },
        }, () => React.createElement(MinimalSettingsContent)))
