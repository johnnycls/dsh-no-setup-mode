// ============================================================================
// dsh-minimal-mode — DeepSeek Harness 極簡模式（Client bundle）
// ----------------------------------------------------------------------------
...
// 功能：
//   - 安裝後自動進入極簡模式（enabled 恆啟用；「離開極簡模式」僅影響當前會話）
//   - 隱藏 Logo（CSS）、tab、Session Log、輸入框多餘按鈕、設置頁
//   - 極簡設置 Dialog：語言 + DeepSeek API Key（格式校驗後保存）+ 離開
//   - 極簡標題列：聊天 / 工作 / 專家 三模式切換（進入自動選聊天模式）
//   - 極簡輸入列：3 行 textarea（Enter 送出 / 繁忙插話）+ shipped 圓形發送/停止按鈕
//   - 餘額顯示：輸入框下方右對齊（¥CNY · ≈$USD），每輪對話結束自動刷新
  ...
  factory: (require) => {
    ...
    return {
      ...
      apply(ctx) {
        ...
        // 套用極簡偏好：busyEnter=steer（插話）、default=chat、模型 Flash；並記錄原值
        // 全程 try/catch：任何一步失敗都不影響極簡 UI 的註冊與顯示
          ...
          try {
            ...
            if (on) {
              if (!prefsApplied) {
                ...
                const conv = namespaces.find((n) => n.ns === 'ui-conversation')
              const raw = conv !== undefined && conv.value !== undefined && conv.value !== null ? conv.value.busyEnter : undefined
              prevBusyEnter = raw !== undefined && raw !== null ? raw : 'queue'
              prefsApplied = true
            }
            ...
            await rpc(api.settings.update({ ns: 'agent-default-model', patch: { provider: 'deepseek-official', model: 'deepseek-v4-flash' } }))
          } else {
            await rpc(api.settings.update({ ns: 'ui-conversation', patch: { busyEnter: prevBusyEnter } }))
            prefsApplied = false
          }
          } catch (error) {
            console.error('dsh-minimal-mode: applyPrefs failed', error)
          }
        }
        ...
        function MinimalHeader(props) {
          ...
          React.useEffect(() => {
            let alive = true
            if (api !== undefined) {
              rpc(api.settings.describe({})).then((res) => {
                if (!alive) return
                const namespaces = res.value !== undefined && res.value.namespaces !== undefined ? res.value.namespaces : []
                const presets = namespaces.find((n) => n.ns === 'agent-presets')
                const def = presets !== undefined && presets.value !== undefined && presets.value !== null ? presets.value.default : undefined
                setCurrent(def !== undefined && def !== null ? def : 'chat')
              })
            } else {
              setCurrent('chat')
            }
          ...
          const pick = async (id) => {
            ...
            if (api !== undefined) {
              await rpc(api.settings.update({ ns: 'agent-presets', patch: { default: id } }))
              if (model !== undefined) {
                await rpc(api.settings.update({ ns: 'agent-default-model', patch: { provider: 'deepseek-official', model: model } }))
              }
            }
            setCurrent(id)
          }
        ...
        function MinimalComposer(props) {
          ...
          const readBalance = React.useCallback(() => {
            fetch('/minimal-mode/balance', { cache: 'no-store' }).then((res) => (
              res.json()
            )).then((data) => {
          ...
          return React.createElement(React.Fragment, null,
            ...
            React.createElement('div', {
              ...
              // 餘額顯示（與輸入卡片同寬、貼卡片右緣；未設定 key 或尚未取得時不渲染）
              balance !== undefined && balance !== null && balance.ok === true && Array.isArray(balance.balances) && balance.balances.length > 0
                ? React.createElement('div', {
                  style: { boxSizing: 'border-box', width: '100%', maxWidth: 'var(--dsh-composer-card-max-width, 900px)', padding: '4px 20px 6px', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 12, fontSize: 12, lineHeight: '18px', color: 'var(--dsw-alias-label-secondary)' },
                },
                  balance.balances.map((entry) => React.createElement('span', { key: entry.currency },
                    `${entry.currency === 'USD' ? '$' : entry.currency === 'CNY' ? '¥' : ''}${Number(entry.total).toFixed(2)} ${entry.currency}`,
                  )),
                )
                : balance !== undefined && balance !== null && balance.ok === false && balance.error !== undefined
                  ? React.createElement('div', {
                    style: { boxSizing: 'border-box', width: '100%', maxWidth: 'var(--dsh-composer-card-max-width, 900px)', padding: '4px 20px 6px', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', fontSize: 12, lineHeight: '18px', color: 'var(--dsw-alias-label-secondary)' },
                  },
        ...
        const applyMode = async () => {
          ...
          disposers = []
          if (enabled) {
            // 先註冊極簡 UI（不阻塞於偏好套用；偏好失敗不影響進入極簡模式）
            try {
              ...
              console.error('dsh-minimal-mode: register minimal UI failed', error)
            }
            ...
            applyPrefs(false).catch((error) => {
              console.error('dsh-minimal-mode: applyPrefs(restore) failed', error)
            })
          }
        }
