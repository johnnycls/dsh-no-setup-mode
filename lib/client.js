    ...
    return {
      ...
      apply(ctx) {
        ...
        function MinimalComposer(props) {
          ...
          // ---- 餘額顯示：每輪對話結束時請 host 刷新（refreshTick），稍後重讀 ----
          ...
          const readBalance = React.useCallback(() => {
            ...
            rpc(api.settings.describe({})).then((res) => {
              const namespaces = res.value !== undefined && res.value.namespaces !== undefined ? res.value.namespaces : []
              const ns = namespaces.find((n) => n.ns === 'minimal-balance')
              const value = ns !== undefined && ns.value !== undefined && ns.value !== null ? ns.value : undefined
              if (value !== undefined && (value.cny !== undefined || value.error !== undefined)) {
                setBalance(value)
              }
            }).catch(() => {})
          }, [])
          ...
          React.useEffect(() => {
            readBalance()
            // 尚未取得餘額（如 key 剛設定、host 尚未寫入）時輕量輪詢
          ...
          React.useEffect(() => {
            ...
            if (wasRunning === true && running === false) {
              // 通知 host 刷新餘額（tick 單調遞增，必定觸發 settings/updated）
              if (api !== undefined) {
                rpc(api.settings.update({ ns: 'minimal-mode', patch: { refreshTick: Date.now() } }))
              }
              const timer = setTimeout(() => { readBalance() }, 1200)
          ...
          return React.createElement(React.Fragment, null,
            ...
            // 餘額顯示（輸入框下方，右對齊；未設定 key 或尚未取得時不渲染）
            balance !== undefined && balance !== null
              ? React.createElement('div', {
                style: { padding: '2px 24px 12px', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 12, fontSize: 12, lineHeight: '18px', color: 'var(--dsw-alias-label-secondary)' },
              },
                balance.error !== undefined && balance.error !== ''
                  ? React.createElement('span', null, '餘額獲取失敗')
                  : React.createElement(React.Fragment, null,
                    React.createElement('span', null, `¥${balance.cny} CNY`),
                    React.createElement('span', null, `≈$${balance.usd} USD`),
                  ),
              )
              : null,
          )
